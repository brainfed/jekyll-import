  ...
  module Importers
    class Podcast < Importer
      def self.specify_options(c)
        ...
        c.option "render_audio",   "--render_audio",     "Render `<audio>` element in posts for the enclosure URLs. (default: true)"
      ...
      def self.write_rss_item(item, options)
        ...
        render_audio = options.fetch("render_audio", true)

        formatted_date = item.date.strftime("%Y-%m-%d")
        post_name = Jekyll::Utils.slugify(item.title.split(':').first, :mode => "latin")
